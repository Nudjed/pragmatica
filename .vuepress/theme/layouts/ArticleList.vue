        ...
        <div class="w-1/4">
          <div class="p-4 sm:max-w-xs my-4 bg-white">
           ...
           <div v-for="(tag,index) in tags" :key="index" class="mb-3">
            <input class="text-xl text-grey-dark font-sans-serif font-normal" type="checkbox"  v-model="tag.checked" v-on:change="getfilteredData">
            <label class="mr-4">
              {{ tag.value }}
            </label>
          </div>
    ...
    methods: {
      getfilteredData: function() {
        ...
        if (this.selectedFilters.length > 0) {
          ...
          filteredDataByfilters= this.filteredData.filter(obj => this.selectedFilters.every(val => obj.frontmatter.tags.indexOf(val) >= 0));
